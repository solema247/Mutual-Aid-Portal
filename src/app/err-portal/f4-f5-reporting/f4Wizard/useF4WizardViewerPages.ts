...
/**
 * Renders PDF pages or a single image into data URLs for the wizard viewer (same logic as before extraction).
 */
export function useF4WizardViewerPages ({
  step,
  fileUrl,
  file,
  tempKey,
        ...
        if (isImage) {
          ...
          if (!cancelled) {
            ...
            setWizardPages([
              {
                dataUrl,
                displayWidth: probe.naturalWidth,
                displayHeight: probe.naturalHeight,
              },
            ])
          }
