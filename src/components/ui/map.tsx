...
> = {
  ...
  /**
   * Maximum zoom level when zooming to a cluster (e.g. 11 = state/city level).
   * Prevents zooming in to street level. If not set, uses the cluster's expansion zoom.
   */
  clusterZoomMax?: number;
  /**
   * When clusterZoomMax caps the zoom, called with the cluster's leaf features
   * so the parent can show a popup with links to each project (cluster doesn't expand to points).
   */
  onClusterClickWithLeaves?: (
    coordinates: [number, number],
    pointCount: number,
...
>({
  ...
  useEffect(() => {
    ...
    const handleClusterClick = async (
      ...
      const zoomCapped =
        clusterZoomMax != null && expansionZoom > clusterZoomMax;
      const zoom = zoomCapped ? clusterZoomMax! : expansionZoom;

      if (zoomCapped && onClusterClickWithLeaves) {
        try {
          const leaves = await source.getClusterLeaves(clusterId, 500, 0);
          const features = Array.isArray(leaves) ? leaves as GeoJSON.Feature<GeoJSON.Point, P>[] : [];
          onClusterClickWithLeaves(coordinates, pointCount, features);
        } catch {
          // ignore if getClusterLeaves not available or fails
        }
      }
