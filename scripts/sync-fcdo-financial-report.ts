/**
 * sync-fcdo-financial-report.ts
 *
 * Syncs sheets from an FCDO Financial Report Excel file into Supabase:
 *
 *   1. "Spending Summary" → err_summary + err_expense (sector aggregates)
 *      - One err_summary per F1 serial (skipped if already exists)
 *      - One err_expense row per non-zero sector per F1 serial
 *      - Handles both 5-part historical serials (via activities_raw_import)
 *        and 4-part portal serials (via err_projects.grant_id with zero-padding)
 *
 *   2. "Disbursement Overview" → activities_raw_import (upsert by Serial Number)
 *      - Also updates err_projects for 4-part (portal-format) serials:
 *        date_transfer, f4_status, date_report_completed
 *        (only written where portal currently has no data / status = 'waiting')
 *
 *   3. "F4s Report" → err_expense (individual receipt line items)
 *      - Each row inserted as a detailed expense under its serial's err_summary
 *      - Adds alongside existing aggregate entries (no deletion)
 *      - Skips serials already loaded in this session (idempotent)
 *      - Handles both historical (activities_raw_import_id) and portal (project_id)
 *
 * Serial matching (per LoHub/Hamza feedback):
 *   Sheet serials and portal grant_ids share donor + state + workplan number,
 *   but may differ in (a) month segment (e.g. 0226 vs 0326) and (b) Khartoum
 *   sub-locality codes (KR/OD/BH/JB/EN/OM in sheet vs KH in portal).
 *   Example: LCC-FCDO-KR-0226-0058 (sheet) -> LCC-FCDO-KH-0326-0058 (portal)
 *   The resolver matches exact serials first, then month-agnostic keys with
 *   unique greedy assignment (preferring the closest portal month >= sheet month).
 *
 * Usage (local Excel file):
 *   npx ts-node --project tsconfig.scripts.json scripts/sync-fcdo-financial-report.ts \
 *     --file "docs/Compliance check feature/LCC-FCDO-0325-0001 Financial Report  (1).xlsx"
 *
 * Usage (Google Sheet, e.g. LCC-FCDO-0325-0002 report):
 *   npx ts-node --project tsconfig.scripts.json scripts/sync-fcdo-financial-report.ts \
 *     --sheet-id 1R-Zuor0FqqypjSpzIKQd1cfgrZxh0fso3Ki9eb8qUb8
 *   (requires GOOGLE_SHEETS env JSON or GOOGLE_VISION_FILE service account with
 *    Viewer access to the sheet)
 *
 * F4-only (matched portal projects → err_summary / err_expense / err_projects F4
 * fields; uses portal grant_id via project_id; skips activities_raw_import):
 *   ... --sheet-id <id> --f4-only
 *
 * Prerequisite: Run sql/add_disbursement_paid_to.sql in the Supabase SQL editor first.
 */

import * as fs from 'fs'
import * as path from 'path'
import * as XLSX from 'xlsx'
import { createClient } from '@supabase/supabase-js'
import { google } from 'googleapis'

// eslint-disable-next-line @typescript-eslint/no-explicit-any
type AnySupabase = any

// ── Config ──────────────────────────────────────────────────────────────────

const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL || ''
const SUPABASE_SERVICE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || ''
const BATCH_SIZE = 50

// Sector columns in Spending Summary (columns B–P, index 1–15)
const SECTOR_COLUMNS = [
  { col: 1, name: 'Protection' },
  { col: 2, name: 'Shelter & NFIs' },
  { col: 3, name: 'WASH' },
  { col: 4, name: 'Food Security' },
  { col: 5, name: 'Health' },
  { col: 6, name: 'Support logistic operations' },
  { col: 7, name: 'Volunteer Support' },
  { col: 8, name: 'Women & children needs' },
  { col: 9, name: 'Mental and physical health' },
  { col: 10, name: 'Education' },
  { col: 11, name: 'Capacity Building' },
  { col: 12, name: 'Livelihoods' },
  { col: 13, name: 'Peacebuilding / Social Cohesion' },
  { col: 14, name: 'Youth Space' },
  { col: 15, name: 'Socioeconomic' },
]

// ── Helpers ──────────────────────────────────────────────────────────────────

function parseArgs(): { file: string; sheetId: string; dryRun: boolean; f4Only: boolean } {
  const args = process.argv.slice(2)
  let file = ''
  let sheetId = ''
  let dryRun = false
  let f4Only = false
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--file' && args[i + 1]) file = args[++i]
    if (args[i] === '--sheet-id' && args[i + 1]) sheetId = args[++i]
    if (args[i] === '--dry-run') dryRun = true
    if (args[i] === '--f4-only') f4Only = true
  }
  if (!file && !sheetId) {
    console.error('Error: --file <path> or --sheet-id <googleSheetId> is required')
    process.exit(1)
  }
  return { file, sheetId, dryRun, f4Only }
}

function toNumber(val: unknown): number | null {
  if (val === null || val === undefined || val === '') return null
  if (typeof val === 'number') return isNaN(val) ? null : val
  // Google Sheets returns formatted strings like "$3,250.00" or "100.00%"
  const cleaned = String(val).replace(/[$,%\s]/g, '')
  if (cleaned === '' || cleaned === '-') return null
  const n = Number(cleaned)
  return isNaN(n) ? null : n
}

const MONTHS: Record<string, string> = {
  jan: '01', feb: '02', mar: '03', apr: '04', may: '05', jun: '06',
  jul: '07', aug: '08', sep: '09', oct: '10', nov: '11', dec: '12',
}

function toDateString(val: unknown): string | null {
  if (val === null || val === undefined || val === '') return null
  // XLSX stores dates as serial numbers or strings
  if (typeof val === 'number') {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const date = (XLSX.SSF as any).parse_date_code(val)
    if (date) {
      const y = date.y
      const m = String(date.m).padStart(2, '0')
      const d = String(date.d).padStart(2, '0')
      return `${y}-${m}-${d}`
    }
  }
  if (typeof val === 'string') {
    const s = val.trim()
    if (!s) return null
    // Already YYYY-MM-DD
    if (/^\d{4}-\d{2}-\d{2}/.test(s)) return s.substring(0, 10)
    // DD-Mon-YYYY (Google Sheets format, e.g. "23-Feb-2026")
    const dMonY = s.match(/^(\d{1,2})-([A-Za-z]{3})[A-Za-z]*-(\d{4})$/)
    if (dMonY) {
      const mm = MONTHS[dMonY[2].toLowerCase()]
      if (mm) return `${dMonY[3]}-${mm}-${dMonY[1].padStart(2, '0')}`
    }
    // DD/MM/YYYY
    const dmyMatch = s.match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})/)
    if (dmyMatch) return `${dmyMatch[3]}-${dmyMatch[2].padStart(2, '0')}-${dmyMatch[1].padStart(2, '0')}`
    // Try native Date parse
    const d = new Date(s)
    if (!isNaN(d.getTime())) return d.toISOString().substring(0, 10)
  }
  return null
}

function isValidSerial(serial: unknown): serial is string {
  return typeof serial === 'string' && /^LCC-/.test(serial.trim())
}

/**
 * Returns true for 4-part serials that correspond to portal-uploaded projects
 * (e.g. LCC-FCDO-JA-0126-488). 5-part/6-part serials are historical.
 */
function isPortalSerial(serial: string): boolean {
  return serial.split('-').length === 5
}

// ── Serial resolver (LoHub matching rules) ────────────────────────────────────

/**
 * Khartoum sub-locality codes used in LoHub sheets. The portal registers all
 * of these as state code KH (e.g. sheet LCC-FCDO-KR-0226-0058 = portal
 * LCC-FCDO-KH-0326-0058).
 */
const KHARTOUM_CODES = new Set(['KR', 'OD', 'BH', 'JB', 'EN', 'OM', 'KH'])

/** Month-agnostic matching key: donor | state (Khartoum-mapped) | padded number */
function serialKey(serial: string): string | null {
  const parts = String(serial).trim().split('-')
  if (parts.length !== 5) return null
  let [, donor, state, , num] = parts
  if (KHARTOUM_CODES.has(state)) state = 'KH'
  return `${donor}|${state}|${num.padStart(4, '0')}`
}

/** Extract MM from the MMYY segment; returns 99 when unparseable (sorts last). */
function serialMonth(serial: string): number {
  const mmyy = String(serial).trim().split('-')[3] || ''
  const mm = parseInt(mmyy.slice(0, 2), 10)
  return Number.isFinite(mm) ? mm : 99
}

export interface PortalProjectRef {
  projectId: string
  grantId: string
}

/**
 * Builds a map from sheet serial -> portal project using LoHub's matching rules:
 *   1. Exact grant_id match wins.
 *   2. Otherwise match on donor + state (Khartoum sub-codes -> KH) + workplan
 *      number, ignoring the month segment. When several portal projects share
 *      that key (numbers reused across months), assign greedily by smallest
 *      month distance, preferring portal month >= sheet month, each portal
 *      project used at most once.
 */
function buildSerialResolver(
  sheetSerials: string[],
  portalProjects: { id: string; grant_id: string | null }[]
): Map<string, PortalProjectRef> {
  const resolved = new Map<string, PortalProjectRef>()

  const exactByGrantId = new Map<string, PortalProjectRef>()
  const byKey = new Map<string, PortalProjectRef[]>()
  for (const p of portalProjects) {
    if (!p.grant_id) continue
    const gid = String(p.grant_id).trim()
    const ref: PortalProjectRef = { projectId: String(p.id), grantId: gid }
    exactByGrantId.set(gid, ref)
    const key = serialKey(gid)
    if (key) {
      if (!byKey.has(key)) byKey.set(key, [])
      byKey.get(key)!.push(ref)
    }
  }

  const usedProjectIds = new Set<string>()
  const pending: { serial: string; candidates: PortalProjectRef[] }[] = []

  for (const serial of new Set(sheetSerials)) {
    const exact = exactByGrantId.get(serial)
    if (exact) {
      resolved.set(serial, exact)
      usedProjectIds.add(exact.projectId)
      continue
    }
    const key = serialKey(serial)
    const candidates = key ? byKey.get(key) || [] : []
    if (candidates.length > 0) pending.push({ serial, candidates })
  }

  // Greedy unique assignment: smallest month delta first, portal month >= sheet
  // month preferred over earlier months (portal registration happens after
  // disbursement listing).
  const pairs: { serial: string; ref: PortalProjectRef; score: number }[] = []
  for (const { serial, candidates } of pending) {
    const sheetMm = serialMonth(serial)
    for (const ref of candidates) {
      const delta = serialMonth(ref.grantId) - sheetMm
      // Forward shifts score 0,1,2,...; backward shifts are penalised.
      const score = delta >= 0 ? delta : 100 - delta
      pairs.push({ serial, ref, score })
    }
  }
  pairs.sort((a, b) => a.score - b.score)
  for (const { serial, ref } of pairs) {
    if (resolved.has(serial) || usedProjectIds.has(ref.projectId)) continue
    resolved.set(serial, ref)
    usedProjectIds.add(ref.projectId)
  }

  return resolved
}

/**
 * Normalise Excel F4 status strings to portal enum values.
 */
function normaliseF4Status(excelStatus: string | null): string | null {
  if (!excelStatus) return null
  const s = excelStatus.trim().toLowerCase()
  if (s === 'completed') return 'completed'
  if (s.startsWith('partial')) return 'partial'
  if (s === 'waiting') return 'waiting'
  if (s === 'in progress') return 'in review'
  return s
}

/**
 * Load an entire table with pagination and retry on transient network errors.
 */
async function fetchAllRows(
  supabase: AnySupabase,
  table: string,
  cols: string
): Promise<any[]> {
  const out: any[] = []
  for (let page = 0; ; page++) {
    let lastError: unknown = null
    let data: any[] | null = null
    for (let attempt = 0; attempt < 3; attempt++) {
      const res = await supabase
        .from(table)
        .select(cols)
        .range(page * 1000, (page + 1) * 1000 - 1)
      if (!res.error) { data = res.data; lastError = null; break }
      lastError = res.error
      await new Promise((r) => setTimeout(r, 1000 * (attempt + 1)))
    }
    if (lastError) throw new Error(`Failed to load ${table}: ${(lastError as any)?.message || lastError}`)
    if (!data || data.length === 0) break
    out.push(...data)
    if (data.length < 1000) break
  }
  return out
}

// ── Excel parsing ─────────────────────────────────────────────────────────────

interface SpendingSummaryRow {
  serial: string
  sectorAmounts: { name: string; amount: number }[]
  totalExpenses: number | null   // col R
  submitPct: number | null       // col S
  totalBudget: number | null     // col T
  balance: number | null         // col U
  state: string | null           // col V
}

interface DisbursementRow {
  serial: string
  errName: string | null
  sdg: number | null
  usd: number | null
  rate: number | null
  paidTo: string | null
  transferDate: string | null
  f4Status: string | null
  state: string | null
  reportSubmit: string | null
  lhubComment: string | null
}

function parseSpendingSummary(rows: unknown[][]): SpendingSummaryRow[] {
  // Row 1 = sector group headers, Row 2 = column headers, Rows 3+ = data
  const results: SpendingSummaryRow[] = []

  // Data starts at row index 2 (0-based) = row 3 in Excel
  for (let i = 2; i < rows.length; i++) {
    const row = rows[i] as unknown[]
    const serial = row[0]
    if (!isValidSerial(serial)) continue

    const sectorAmounts: { name: string; amount: number }[] = []
    for (const { col, name } of SECTOR_COLUMNS) {
      const amt = toNumber(row[col])
      if (amt !== null && amt > 0) {
        sectorAmounts.push({ name, amount: amt })
      }
    }

    results.push({
      serial: String(serial).trim(),
      sectorAmounts,
      totalExpenses: toNumber(row[17]),  // col R (0-indexed = 17)
      submitPct: toNumber(row[18]),       // col S
      totalBudget: toNumber(row[19]),     // col T
      balance: toNumber(row[20]),         // col U
      state: row[21] ? String(row[21]).trim() : null,  // col V
    })
  }

  return results
}

function parseDisbursementOverview(rows: unknown[][]): DisbursementRow[] {
  const results: DisbursementRow[] = []

  // Row 0 = headers, Rows 1+ = data
  for (let i = 1; i < rows.length; i++) {
    const row = rows[i] as unknown[]
    const serial = row[0]
    if (!isValidSerial(serial)) continue

    results.push({
      serial: String(serial).trim(),
      errName: row[1] ? String(row[1]).trim() : null,
      sdg: toNumber(row[2]),
      usd: toNumber(row[3]),
      rate: toNumber(row[4]),
      paidTo: row[5] ? String(row[5]).trim() : null,
      transferDate: toDateString(row[6]),
      f4Status: row[7] ? String(row[7]).trim() : null,
      state: row[8] ? String(row[8]).trim() : null,
      reportSubmit: toDateString(row[9]),
      // col K (index 10) = Activity Duration — computed, skipped
      lhubComment: row[11] ? String(row[11]).trim() : null,
    })
  }

  return results
}

// ── Main sync logic ───────────────────────────────────────────────────────────

async function syncSpendingSummary(
  supabase: AnySupabase,
  rows: SpendingSummaryRow[],
  dryRun: boolean,
  resolver: Map<string, PortalProjectRef>
) {
  console.log(`\n=== Spending Summary: ${rows.length} F1 serials ===`)

  // Fetch all activities_raw_import records for fast in-memory lookup
  console.log('Loading activities_raw_import records...')
  const rawImports = await fetchAllRows(supabase, 'activities_raw_import', 'id, "Serial Number"')

  const serialToImportId = new Map<string, string>()
  for (const rec of rawImports || []) {
    const sn = rec['Serial Number']
    if (sn) serialToImportId.set(String(sn).trim(), String(rec.id))
  }
  console.log(`  Loaded ${serialToImportId.size} activities_raw_import records`)

  // Fetch existing err_summary records to skip duplicates
  console.log('Loading existing err_summary records...')
  const existingSummaries = await fetchAllRows(supabase, 'err_summary', 'id, activities_raw_import_id, project_id')

  const existingByImportId = new Set<string>()
  const existingByProjectId = new Set<string>()
  for (const s of existingSummaries || []) {
    if (s.activities_raw_import_id) existingByImportId.add(String(s.activities_raw_import_id))
    if (s.project_id) existingByProjectId.add(String(s.project_id))
  }
  console.log(`  Found ${existingByImportId.size + existingByProjectId.size} existing summaries`)

  const stats = {
    matched: 0,
    unmatched: 0,
    skippedExisting: 0,
    summariesCreated: 0,
    expensesCreated: 0,
    errors: 0,
  }

  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    const batch = rows.slice(i, i + BATCH_SIZE)

    for (const row of batch) {
      // Portal match: attach to project_id (portal grant_id). Prefer portal path
      // so sheet serials are not carried as activities_raw_import identity.
      const projectId = resolver.get(row.serial)?.projectId
      const importId = projectId ? undefined : serialToImportId.get(row.serial)

      if (!importId && !projectId) {
        console.warn(`  [UNMATCHED] ${row.serial}`)
        stats.unmatched++
        continue
      }
      stats.matched++

      // Skip if summary already exists
      if (importId && existingByImportId.has(importId)) {
        stats.skippedExisting++
        continue
      }
      if (projectId && existingByProjectId.has(projectId)) {
        stats.skippedExisting++
        continue
      }

      const portalGrantId = projectId ? resolver.get(row.serial)?.grantId : undefined
      if (dryRun) {
        console.log(
          `  [DRY RUN] Would create summary for ${row.serial}` +
            (portalGrantId ? ` → ${portalGrantId}` : '') +
            ` (${row.sectorAmounts.length} sectors)`
        )
        stats.summariesCreated++
        stats.expensesCreated += row.sectorAmounts.length
        continue
      }

      try {
        // Insert err_summary
        const summaryPayload: Record<string, unknown> = {
          activities_raw_import_id: importId || null,
          project_id: projectId || null,
          total_expenses: row.totalExpenses,
          total_grant: row.totalBudget,
          remainder: row.balance,
          language: 'en',
          review_status: 'pending_review',
        }

        const { data: inserted, error: insErr } = await supabase
          .from('err_summary')
          .insert(summaryPayload)
          .select('id')
          .single()

        if (insErr) throw insErr
        const summary_id = inserted.id
        stats.summariesCreated++

        // Track so we don't try to insert again in this run
        if (importId) existingByImportId.add(importId)
        if (projectId) existingByProjectId.add(projectId)

        // Insert err_expense rows (one per sector)
        if (row.sectorAmounts.length > 0) {
          const expensePayload = row.sectorAmounts.map(({ name, amount }) => ({
            activities_raw_import_id: importId || null,
            project_id: projectId || null,
            summary_id,
            expense_activity: name,
            expense_amount: amount,
            language: 'en',
          }))

          const { error: expErr } = await supabase
            .from('err_expense')
            .insert(expensePayload)

          if (expErr) throw expErr
          stats.expensesCreated += expensePayload.length
        }
      } catch (e: unknown) {
        const msg = e instanceof Error
          ? e.message
          // Supabase errors are plain objects with .message
          : (e as any)?.message || (e as any)?.details || JSON.stringify(e)
        console.error(`  [ERROR] ${row.serial}: ${msg}`)
        stats.errors++
      }
    }

    const processed = Math.min(i + BATCH_SIZE, rows.length)
    process.stdout.write(`\r  Progress: ${processed}/${rows.length}`)
  }

  console.log('\n')
  console.log('Spending Summary results:')
  console.log(`  Matched to DB:     ${stats.matched}`)
  console.log(`  Unmatched:         ${stats.unmatched}`)
  console.log(`  Skipped (existing):${stats.skippedExisting}`)
  console.log(`  Summaries created: ${stats.summariesCreated}`)
  console.log(`  Expenses created:  ${stats.expensesCreated}`)
  console.log(`  Errors:            ${stats.errors}`)
}

async function syncDisbursementOverview(
  supabase: AnySupabase,
  rows: DisbursementRow[],
  dryRun: boolean,
  hasPaidToColumn: boolean,
  resolver: Map<string, PortalProjectRef>
) {
  console.log(`\n=== Disbursement Overview: ${rows.length} rows ===`)

  // Preload existing serials once (per-row lookups are slow and flaky)
  const existingBySerial = new Map<string, string>()
  for (let page = 0; ; page++) {
    const { data, error } = await supabase
      .from('activities_raw_import')
      .select('id, "Serial Number"')
      .range(page * 1000, (page + 1) * 1000 - 1)
    if (error) throw new Error(`Failed to load activities_raw_import: ${error.message}`)
    if (!data || data.length === 0) break
    for (const rec of data) {
      const sn = rec['Serial Number']
      if (sn) existingBySerial.set(String(sn).trim(), String(rec.id))
    }
    if (data.length < 1000) break
  }
  console.log(`  Preloaded ${existingBySerial.size} existing activities_raw_import serials`)

  const stats = {
    updated: 0,
    inserted: 0,
    skippedPortal: 0,
    errors: 0,
  }

  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    const batch = rows.slice(i, i + BATCH_SIZE)

    for (const row of batch) {
      try {
        const existingId = existingBySerial.get(row.serial)

        // Serials that resolve to a portal project live in err_projects — do
        // not create duplicate historical rows for them (existing rows from
        // earlier imports are still updated to stay fresh).
        if (!existingId && resolver.has(row.serial)) {
          stats.skippedPortal++
          continue
        }

        if (dryRun) {
          console.log(`  [DRY RUN] Would ${existingId ? 'update' : 'insert'} ${row.serial} → F4: ${row.f4Status}, USD: ${row.usd}`)
          if (existingId) stats.updated++
          else stats.inserted++
          continue
        }

        const updatePayload: Record<string, unknown> = {
          'ERR Name': row.errName,
          'SDG': row.sdg,
          'USD': row.usd,
          'Rate': row.rate,
          'Date Transfer': row.transferDate,
          'F4': row.f4Status,
          'State': row.state,
          'Date Report Completed': row.reportSubmit,
          'Comments': row.lhubComment,
        }

        if (hasPaidToColumn && row.paidTo !== null) {
          updatePayload['Paid To'] = row.paidTo
        }

        // Remove null values to avoid overwriting existing data with null
        const cleanPayload = Object.fromEntries(
          Object.entries(updatePayload).filter(([, v]) => v !== null && v !== undefined)
        )

        if (existingId) {
          const { error: updErr } = await supabase
            .from('activities_raw_import')
            .update(cleanPayload)
            .eq('id', existingId)

          if (updErr) throw updErr
          stats.updated++
        } else {
          // Insert new record with serial number
          const insertPayload: Record<string, unknown> = { 'Serial Number': row.serial, ...cleanPayload }
          const { error: insErr } = await supabase
            .from('activities_raw_import')
            .insert(insertPayload)

          if (insErr) throw insErr
          stats.inserted++
        }
      } catch (e: unknown) {
        const msg = e instanceof Error
          ? e.message
          : (e as any)?.message || (e as any)?.details || JSON.stringify(e)
        console.error(`  [ERROR] ${row.serial}: ${msg}`)
        stats.errors++
      }
    }

    const processed = Math.min(i + BATCH_SIZE, rows.length)
    process.stdout.write(`\r  Progress: ${processed}/${rows.length}`)
  }

  console.log('\n')
  console.log('Disbursement Overview results:')
  console.log(`  Updated:  ${stats.updated}`)
  console.log(`  Inserted: ${stats.inserted}`)
  console.log(`  Skipped (portal projects): ${stats.skippedPortal}`)
  console.log(`  Errors:   ${stats.errors}`)
}

// ── err_projects update from Disbursement Overview ───────────────────────────

/**
 * For sheet serials that resolve to a portal project, update err_projects
 * fields that are currently missing:
 *   - date_transfer    (only if currently null)
 *   - f4_status        (only if currently null or 'waiting')
 *   - date_report_completed (only if currently null)
 */
async function syncErrProjectsFromDisbursement(
  supabase: AnySupabase,
  rows: DisbursementRow[],
  dryRun: boolean,
  resolver: Map<string, PortalProjectRef>
) {
  console.log('\n=== Updating err_projects from Disbursement Overview ===')

  const fourPartRows = rows.filter(r => isPortalSerial(r.serial))
  console.log(`  4-part (portal-format) serials: ${fourPartRows.length}`)

  // Load current state of all portal projects
  const { data: portalProjects, error: ppErr } = await supabase
    .from('err_projects')
    .select('id, grant_id, f4_status, date_transfer, date_report_completed')
  if (ppErr) throw new Error(`Failed to load err_projects: ${ppErr.message}`)

  const projectById = new Map<string, {
    id: string
    grant_id: string | null
    f4_status: string | null
    date_transfer: string | null
    date_report_completed: string | null
  }>()
  for (const p of portalProjects || []) {
    projectById.set(String(p.id), p)
  }
  console.log(`  Loaded ${projectById.size} portal projects`)

  const stats = { matched: 0, unmatched: 0, updated: 0, skipped: 0, errors: 0 }

  for (const row of fourPartRows) {
    const ref = resolver.get(row.serial)
    const project = ref ? projectById.get(ref.projectId) : undefined

    if (!project) {
      stats.unmatched++
      continue
    }
    stats.matched++
    const paddedId = `${row.serial} → ${project.grant_id}`

    const updatePayload: Record<string, unknown> = {}

    // date_transfer — only if portal has none
    if (!project.date_transfer && row.transferDate) {
      updatePayload.date_transfer = row.transferDate
    }

    // f4_status — only if portal shows waiting or has nothing
    const normF4 = normaliseF4Status(row.f4Status)
    if (normF4 && normF4 !== 'waiting' && (!project.f4_status || project.f4_status === 'waiting')) {
      updatePayload.f4_status = normF4
    }

    // date_report_completed — only if portal has none
    if (!project.date_report_completed && row.reportSubmit) {
      updatePayload.date_report_completed = row.reportSubmit
    }

    if (Object.keys(updatePayload).length === 0) {
      stats.skipped++
      continue
    }

    if (dryRun) {
      console.log(`  [DRY RUN] ${paddedId}: ${JSON.stringify(updatePayload)}`)
      stats.updated++
      continue
    }

    try {
      const { error: updErr } = await supabase
        .from('err_projects')
        .update(updatePayload)
        .eq('id', project.id)
      if (updErr) throw updErr
      stats.updated++
    } catch (e: unknown) {
      const msg = e instanceof Error
        ? e.message
        : (e as any)?.message || (e as any)?.details || JSON.stringify(e)
      console.error(`  [ERROR] ${paddedId}: ${msg}`)
      stats.errors++
    }
  }

  console.log('\nerr_projects update results:')
  console.log(`  Matched:                      ${stats.matched}`)
  console.log(`  Unmatched (not in portal):    ${stats.unmatched}`)
  console.log(`  Updated:                      ${stats.updated}`)
  console.log(`  Skipped (data already exists):${stats.skipped}`)
  console.log(`  Errors:                       ${stats.errors}`)
}

// ── F4s Report ───────────────────────────────────────────────────────────────

interface F4sReportRow {
  serial: string
  activity: string | null
  paymentDate: string | null
  description: string | null
  seller: string | null
  paymentMethod: string | null
  amountSdg: number | null
  amountUsd: number | null
}

function parseF4sReport(rows: unknown[][]): F4sReportRow[] {
  // Row 0 = headers, Rows 1+ = data
  // Cols: A=Serial, B=Activity, C=PaymentDate, D=Description, E=Seller, F=Payment Method,
  //       G=Expenditure SDG, H=Exg Rate, I=Expenditure USD, J=LHub Comment
  const results: F4sReportRow[] = []

  for (let i = 1; i < rows.length; i++) {
    const row = rows[i] as unknown[]
    if (!isValidSerial(row[0])) continue

    results.push({
      serial: String(row[0]).trim(),
      activity: row[1] ? String(row[1]).trim() : null,
      paymentDate: toDateString(row[2]),
      description: row[3] ? String(row[3]).trim() : null,
      seller: row[4] ? String(row[4]).trim() : null,
      paymentMethod: row[5] ? String(row[5]).trim() : null,
      amountSdg: toNumber(row[6]),
      amountUsd: toNumber(row[8]),  // col I (index 8), skip col H (exchange rate)
    })
  }

  return results
}

async function syncF4sReport(
  supabase: AnySupabase,
  rows: F4sReportRow[],
  dryRun: boolean,
  resolver: Map<string, PortalProjectRef>
) {
  console.log(`\n=== F4s Report: ${rows.length} line items ===`)

  // ── Historical path: serial → activities_raw_import_id → summary_id ──────
  const { data: imports, error: impErr } = await supabase
    .from('activities_raw_import')
    .select('id, "Serial Number"')
  if (impErr) throw new Error(`Failed to load activities_raw_import: ${impErr.message}`)

  const serialToImportId = new Map<string, string>()
  for (const rec of imports || []) {
    const sn = rec['Serial Number']
    if (sn) serialToImportId.set(String(sn).trim(), String(rec.id))
  }

  // Load all err_summary records (both historical and portal-linked)
  console.log('Loading err_summary records...')
  const { data: summaries, error: sumErr } = await supabase
    .from('err_summary')
    .select('id, activities_raw_import_id, project_id')
  if (sumErr) throw new Error(`Failed to load err_summary: ${sumErr.message}`)

  const importIdToSummaryId = new Map<string, number>()
  const projectIdToSummaryId = new Map<string, number>()
  for (const s of summaries || []) {
    if (s.activities_raw_import_id) importIdToSummaryId.set(String(s.activities_raw_import_id), Number(s.id))
    if (s.project_id) projectIdToSummaryId.set(String(s.project_id), Number(s.id))
  }

  // Check which summaries already have detailed expenses (skip re-import)
  const { data: existingDetailed } = await supabase
    .from('err_expense')
    .select('summary_id')
    .not('expense_description', 'is', null)

  const summaryIdsWithDetail = new Set<number>(
    (existingDetailed || []).map((r: any) => r.summary_id).filter(Boolean)
  )
  console.log(`  Loaded ${serialToImportId.size} import records, ${importIdToSummaryId.size + projectIdToSummaryId.size} summaries`)
  console.log(`  Summaries already with detailed expenses: ${summaryIdsWithDetail.size}`)

  // Group rows by serial
  const bySerial = new Map<string, F4sReportRow[]>()
  for (const row of rows) {
    const existing = bySerial.get(row.serial) || []
    existing.push(row)
    bySerial.set(row.serial, existing)
  }

  const stats = {
    serialsMatched: 0,
    serialsUnmatched: 0,
    serialsSkipped: 0,
    expensesInserted: 0,
    errors: 0,
  }

  let processed = 0
  for (const [serial, serialRows] of bySerial) {
    processed++
    process.stdout.write(`\r  Progress: ${processed}/${bySerial.size}`)

    let importId: string | undefined
    let projectId: string | undefined
    let summaryId: number | undefined

    const ref = resolver.get(serial)
    if (ref) {
      // Serial resolves to a portal project (handles month shifts / KH codes).
      // F4 expenses attach via project_id (portal grant_id) — sheet serial not stored.
      projectId = ref.projectId
      summaryId = projectIdToSummaryId.get(projectId)
    } else {
      // Historical path via activities_raw_import
      importId = serialToImportId.get(serial)
      if (importId) summaryId = importIdToSummaryId.get(importId)
    }

    if (!summaryId) {
      stats.serialsUnmatched++
      continue
    }
    stats.serialsMatched++

    if (summaryIdsWithDetail.has(summaryId)) {
      stats.serialsSkipped++
      continue
    }

    if (dryRun) {
      console.log(
        `  [DRY RUN] Would insert ${serialRows.length} expenses for ${serial}` +
          (ref ? ` → ${ref.grantId}` : '')
      )
      stats.expensesInserted += serialRows.length
      continue
    }

    try {
      const payload = serialRows.map((row) => ({
        activities_raw_import_id: importId || null,
        project_id: projectId || null,
        summary_id: summaryId,
        expense_activity: row.activity,
        expense_description: row.description,
        expense_amount: row.amountUsd,
        expense_amount_sdg: row.amountSdg,
        payment_date: row.paymentDate,
        payment_method: row.paymentMethod,
        seller: row.seller,
        language: 'en',
      }))

      for (let i = 0; i < payload.length; i += 200) {
        const chunk = payload.slice(i, i + 200)
        const { error: insErr } = await supabase.from('err_expense').insert(chunk)
        if (insErr) throw insErr
      }

      stats.expensesInserted += serialRows.length
      summaryIdsWithDetail.add(summaryId)
    } catch (e: unknown) {
      const msg = e instanceof Error
        ? e.message
        : (e as any)?.message || (e as any)?.details || JSON.stringify(e)
      console.error(`  [ERROR] ${serial}: ${msg}`)
      stats.errors++
    }
  }

  console.log('\n')
  console.log('F4s Report results:')
  console.log(`  Serials matched:                  ${stats.serialsMatched}`)
  console.log(`  Serials unmatched:                ${stats.serialsUnmatched}`)
  console.log(`  Serials skipped (already loaded): ${stats.serialsSkipped}`)
  console.log(`  Expenses inserted:                ${stats.expensesInserted}`)
  console.log(`  Errors:                           ${stats.errors}`)
}

// ── Data loading (Excel file or Google Sheet) ────────────────────────────────

const TAB_NAMES = ['Spending Summary', 'Disbursement Overview', 'F4s Report'] as const

function loadRowsFromExcel(filePath: string): Record<string, unknown[][]> {
  const workbook = XLSX.readFile(filePath)
  console.log('Sheets found:', workbook.SheetNames.join(', '))
  const out: Record<string, unknown[][]> = {}
  for (const tab of TAB_NAMES) {
    const sheet = workbook.Sheets[tab]
    out[tab] = sheet ? XLSX.utils.sheet_to_json(sheet, { header: 1, defval: null }) : []
  }
  return out
}

function loadGoogleCredentials(): Record<string, unknown> {
  let raw = process.env.GOOGLE_SHEETS || ''
  if (!raw && process.env.GOOGLE_VISION_FILE) {
    raw = fs.readFileSync(path.resolve(process.env.GOOGLE_VISION_FILE.trim()), 'utf8')
  }
  if (!raw) {
    throw new Error('Set GOOGLE_SHEETS (inline JSON) or GOOGLE_VISION_FILE (path) for Google Sheets access.')
  }
  const creds = JSON.parse(raw) as Record<string, unknown>
  if (typeof creds.private_key === 'string') {
    creds.private_key = (creds.private_key as string).replace(/\\n/g, '\n')
  }
  return creds
}

async function loadRowsFromGoogleSheet(sheetId: string): Promise<Record<string, unknown[][]>> {
  const auth = new google.auth.GoogleAuth({
    credentials: loadGoogleCredentials() as any,
    scopes: ['https://www.googleapis.com/auth/spreadsheets.readonly'],
  })
  const api = google.sheets({ version: 'v4', auth })

  const meta = await api.spreadsheets.get({ spreadsheetId: sheetId })
  const available = (meta.data.sheets || []).map((s) => s.properties?.title).filter(Boolean)
  console.log('Sheets found:', available.join(', '))

  const out: Record<string, unknown[][]> = {}
  for (const tab of TAB_NAMES) {
    if (!available.includes(tab)) {
      out[tab] = []
      continue
    }
    const res = await api.spreadsheets.values.get({ spreadsheetId: sheetId, range: tab })
    out[tab] = (res.data.values || []) as unknown[][]
  }
  return out
}

// ── Entry point ───────────────────────────────────────────────────────────────

async function main() {
  const { file, sheetId, dryRun, f4Only } = parseArgs()

  if (!SUPABASE_URL || !SUPABASE_SERVICE_KEY) {
    console.error('Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY env vars.')
    console.error('Set them in .env.local or export them before running.')
    process.exit(1)
  }

  const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_KEY)

  if (dryRun) console.log('*** DRY RUN MODE — no writes will occur ***')
  if (f4Only) {
    console.log('*** F4-ONLY MODE — portal-matched serials only; no activities_raw_import writes ***')
    console.log('*** F4 tables use portal grant_id via project_id (sheet serial not stored) ***')
  }

  let tabs: Record<string, unknown[][]>
  if (sheetId) {
    console.log(`Reading Google Sheet: ${sheetId}`)
    tabs = await loadRowsFromGoogleSheet(sheetId)
  } else {
    const filePath = path.resolve(process.cwd(), file)
    if (!fs.existsSync(filePath)) {
      console.error(`File not found: ${filePath}`)
      process.exit(1)
    }
    console.log(`Reading Excel: ${filePath}`)
    tabs = loadRowsFromExcel(filePath)
  }

  if (!tabs['Spending Summary'].length) {
    console.error('Sheet "Spending Summary" not found or empty')
    process.exit(1)
  }
  if (!tabs['Disbursement Overview'].length) {
    console.error('Sheet "Disbursement Overview" not found or empty')
    process.exit(1)
  }

  const spendingRows = parseSpendingSummary(tabs['Spending Summary'])
  console.log(`Parsed ${spendingRows.length} Spending Summary rows`)

  const disbursementRows = parseDisbursementOverview(tabs['Disbursement Overview'])
  console.log(`Parsed ${disbursementRows.length} Disbursement Overview rows`)

  const f4sRows = parseF4sReport(tabs['F4s Report'])
  console.log(`Parsed ${f4sRows.length} F4s Report line items`)

  // Build the serial resolver across all serials in the workbook
  console.log('\nLoading err_projects for serial resolution...')
  const allPortalProjects: { id: string; grant_id: string | null }[] = []
  for (let page = 0; ; page++) {
    const { data, error } = await supabase
      .from('err_projects')
      .select('id, grant_id')
      .range(page * 1000, (page + 1) * 1000 - 1)
    if (error) throw new Error(`Failed to load err_projects: ${error.message}`)
    if (!data || data.length === 0) break
    allPortalProjects.push(...(data as any))
    if (data.length < 1000) break
  }
  console.log(`Loaded ${allPortalProjects.length} portal projects`)

  const allSerials = [
    ...spendingRows.map((r) => r.serial),
    ...disbursementRows.map((r) => r.serial),
    ...f4sRows.map((r) => r.serial),
  ]
  const resolver = buildSerialResolver(allSerials, allPortalProjects)
  const uniqueSheetSerials = new Set(allSerials)
  console.log(`Serial resolver: ${resolver.size}/${uniqueSheetSerials.size} unique sheet serials matched to portal projects`)

  let spendingToSync = spendingRows
  let disbursementToSync = disbursementRows
  let f4sToSync = f4sRows
  if (f4Only) {
    spendingToSync = spendingRows.filter((r) => resolver.has(r.serial))
    disbursementToSync = disbursementRows.filter((r) => resolver.has(r.serial))
    f4sToSync = f4sRows.filter((r) => resolver.has(r.serial))
    console.log(`F4-only matched rows: spending=${spendingToSync.length}, disbursement=${disbursementToSync.length}, f4 line items=${f4sToSync.length}`)
  }

  // Check if "Paid To" column exists
  let hasPaidToColumn = false
  if (!dryRun && !f4Only) {
    const { error: ptErr } = await supabase
      .from('activities_raw_import')
      .select('"Paid To"')
      .limit(1)
    hasPaidToColumn = !ptErr
    if (!hasPaidToColumn) {
      console.warn('\nWARNING: "Paid To" column does not exist in activities_raw_import.')
      console.warn('Run sql/add_disbursement_paid_to.sql in the Supabase SQL editor to add it.')
      console.warn('"Paid To" data will be skipped for now.\n')
    }
  }

  // Run syncs
  // F4-only: write matched portal projects into F4 tables (err_summary/err_expense/
  // err_projects F4 fields) using portal grant_id via project_id. Skip historical
  // activities_raw_import upserts so sheet serials are not stored as identity.
  await syncSpendingSummary(supabase, spendingToSync, dryRun, resolver)
  if (!f4Only) {
    await syncDisbursementOverview(supabase, disbursementToSync, dryRun, hasPaidToColumn, resolver)
  } else {
    console.log('\n(Skipping Disbursement → activities_raw_import in --f4-only mode)')
  }
  await syncErrProjectsFromDisbursement(supabase, disbursementToSync, dryRun, resolver)
  if (f4sToSync.length > 0) {
    await syncF4sReport(supabase, f4sToSync, dryRun, resolver)
  } else {
    console.log('\n(No F4s Report sheet found — skipping)')
  }

  console.log('\nDone.')
}

main().catch((e) => {
  console.error('Fatal error:', e)
  process.exit(1)
})
